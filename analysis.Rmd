...
```{r, echo=FALSE}
df$radius1 <- df$radius1 - mean(df$radius1)
df$radius1 <- df$radius1/sd(df$radius1)/2

df$texture1 <- df$texture1 - mean(df$texture1)
df$texture1 <- df$texture1/sd(df$texture1)/2

df$perimeter1 <- df$perimeter1 - mean(df$perimeter1)
df$perimeter1 <- df$perimeter1/sd(df$perimeter1)/2

df$area1 <- df$area1 - mean(df$area1)
df$area1 <- df$area1/sd(df$area1)/2

df$smoothness1 <- df$smoothness1 - mean(df$smoothness1)
df$smoothness1 <- df$smoothness1/sd(df$smoothness1)/2

df$compactness1 <- df$compactness1 - mean(df$compactness1)
df$compactness1 <- df$compactness1/sd(df$compactness1)/2

df$concavity1 <- df$concave_points1 - mean(df$concavity1)
df$concavity1 <- df$concavity1/sd(df$concavity1)/2

df$concave_points1 <- df$concave_points1 - mean(df$concave_points1)
df$concave_points1 <- df$concave_points1/sd(df$concave_points1)/2

df$symmetry1 <- df$symmetry1 - mean(df$symmetry1)
df$symmetry1 <- df$symmetry1/sd(df$symmetry1)/2

df$fractal_dimension1 <- df$fractal_dimension1 - mean(df$fractal_dimension1)
df$fractal_dimension1 <- df$fractal_dimension1/sd(df$fractal_dimension1)/2
...
bayesian.base_reg.dat <- list(
  N = 500,
  x1 = train_data$radius1,
  x2 = train_data$texture1,
  x3 = train_data$perimeter1,
  x4 = train_data$area1,
  x5 = train_data$smoothness1,
  x6 = train_data$compactness1,
  x7 = train_data$concavity1,
  x8 = train_data$concave_points1,
  x9 = train_data$symmetry1,
  x10 = train_data$fractal_dimension1,
  y = train_data$Diagnosis,
  N_new = 69,
  x1_new = test_data$radius1,
  x2_new = test_data$texture1,
  x3_new = test_data$perimeter1,
  x4_new = test_data$area1,
  x5_new = test_data$smoothness1,
  x6_new = test_data$compactness1,
  x7_new = test_data$concavity1,
  x8_new = test_data$concave_points1,
  x9_new = test_data$symmetry1,
  x10_new = test_data$fractal_dimension1
)
```
...
prior choice?
  ...
  For each coefficient, we assume a Student-t prior distribution with mean 0, degrees-of-freedom parameter v, and scale s, with v and s chosen to provide minimal prior information to constrain the coefficients to lie in a reasonable range. We are motivated to consider the t family because flat-tailed distributions allow for robust inference (Bayesian Data Analysis, p. 412)
  We center our default prior distributions at zero because, in the absence of any problem- specific information, we have no idea if the coefficients $\beta$ will be positive or negative. We now need to choose default values of the scale s and degrees of freedom v in the t prior distributions on the coefficients of the rescaled coefficients. (Bayesian Data Analysis, p. 416)
